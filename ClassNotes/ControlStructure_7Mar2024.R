...
### if conditions is true expression will work
#if (condition) {expression1}
#if (condition) {expression1} else {expression2}
#if (condition1) {expression1} else
#if (condition2) {expression2} else
#if (condition2) {expression2} else if (condition2)
 
x <- 5
if (x==5) print('met the condition') # if x==5 is not true, nothing comes back as a result
...
if (x==3|5) {print('met the condition')
  print('do something further')
}## still prints because the expression is wrong because 5 in this code is defines as logical which comes back as true
...
if (x %in% c(3,5)) {print('met the condition')
  print('do something further')
} else if (x==6) print ('found a 6!') else print('not 3,5, or 6')



# random uniform value
# wrapping in signif function to give us only two digit
z <- signif(runif(1),digits=2)
print(z)

# cat  give a little bit more control to mix text and numbers
# here we use cat to print z value and the message
z > 0.5
...
# λ=10.2 eggs
# 35% chance of parasitism
# 1000 individuals

tester <- runif(1000) # start with random uniform elements
# the results of if else statement ,,,, if it is not greater than >0.35 give us a 0
