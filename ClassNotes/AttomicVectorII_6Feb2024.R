...
my_names <- paste("Genus",seq(1:length(my_vector)),"Species",sep="_")
head (my_vector)
names(my_vector) <- my_names
head(my_vector)
str(my_vector)
...
# first element to repeat and second is the number of repealcations
...
print(my_vec)
seq_along(my_vec) # much faster for models and big data! looking for a vector
seq_len(5) # this is also faster than 1:5, only look at a specific number
...
qplot(x=z) #single input parameter x:qplot
...
sample(x=long_vec, size=11) # it won't work because does not have as many so it has to repeat because we did not tell it to replace
# sample always samples without replacement
...
print(my_weights)
my_weights <- c(100,100,100,100000,1001,100,100,100,100,100)
sample(x=long_vec,replace=TRUE,prob=my_weights) # sampling with replacement and weights

my_weights <- c(100,100,100,500,1001,100,100,100,100,100)
sample(x=long_vec,replace=TRUE,prob=my_weights) # sampling with replacement and weights
...
which(z<3) # it gives us the coefficient of the variable
